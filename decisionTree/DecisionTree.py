...
def entropy(rows):
    ...
    for i in counts:
        ...
        entropy = np.sum(counts[i]*(-1) / float(len(rows)) * np.log2(counts[i] / float(len(rows))))
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
    	rows_counts = class_counts(rows)
        sorted_d = sorted(rows_counts.items(), key=operator.itemgetter(1))  

        self.predictions = sorted_d[-1][0]
...
def prune_tree_depth(node, max_depth):
    """
    Prune strategy:
    if node's depth is larger than min_depth,
    we prune this node's children and make this node as a leaf node, and give its prediction.
    """
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        return node
    ...
    # Call this function recursively on the true branch
    node.true_branch = prune_tree(node.true_branch, max_depth)

    # Call this function recursively on the false branch
    node.false_branch = prune_tree(node.false_branch, max_depth)
...
def print_tree(node, spacing=""):
    ...
    # print (spacing + 'Node depth is: ' + str(node.depth))
    # print (spacing + 'Node id is: ' + str(node.id))
...
def getMaxDepth(node):
    if isinstance(node, Leaf):
        return 0
    return max(getMaxDepth(node.true_branch), getMaxDepth(node.false_branch))+1


def getDepth(node, prunded=[]):

    max = getMaxDepth(node)

    for tmp in range(1,max):
        prunded.append(tmp)

    return prunded


def getPrundedList(node, prunded = []):

    if isinstance(node, Leaf):
        return prunded.append(node.id)

    prunded.append(node.id)

    getPrundedList(node.true_branch)
    getPrundedList(node.false_branch)
